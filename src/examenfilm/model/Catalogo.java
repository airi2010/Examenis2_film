...
public class Catalogo {
    ...
    public HashMap<Integer,Pelicula> orderbyviews(){
        HashMap<Integer,Pelicula> mapviews=new HashMap<>();
        ArrayList<Integer>views =new ArrayList<Integer>();
        for(Pelicula peli: pelimap.values()){
            if(!views.contains(peli.getViews())){
                views.add(peli.getViews());
            }
        }
        Collections.sort(views,Collections.reverseOrder()); 
        int temp=0;
        while(mapviews.size()!= pelimap.size()){
            for(Pelicula peli: pelimap.values()){
                if(peli.getViews()==views.get(temp)){
                    mapviews.put(mapviews.size(),peli);
                }
            }
            temp++;
        }
